...
async def call_model(call: CallbackQuery):
    ...
    if sub is None:
        await call.answer(f"Пустой ответ", show_alert=True)
        return
    if models[msg] not in subs[sub[0]]:
        await call.answer(f"Вам недоступна эта модель. Купите подписку для получения новых моделей.", show_alert=True)
    else:
        answer = await set_model(chat_id, msg)
        if answer:
            await call.answer(f"Модель ИИ изменена на {answer}", show_alert=True)
        else:
            await call.answer(f"У вас уже выбрана эта модель", show_alert=True)
...
async def call_subscription(call: CallbackQuery):
    ...
    if await subs_check(sub[0], msg[0]) < 0:
        await call.answer(f"Вы не можете купить подписку ниже вашей.", show_alert=True)
    else:
        ...
        if answer:
            ...
            await call.answer(f"Ошибка оплаты", show_alert=True)
    await call.message.delete()